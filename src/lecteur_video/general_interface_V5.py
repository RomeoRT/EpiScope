...
interface générale Episcope contenant :
    ...
    - Ajout de la suppression des symptômes 
...
La documentation des fonctions est faite
version : 0.5
"""
...
class InterfaceGenerale():
    ...
    def __init__(self, fenetre):
        ...
        # Partie de création de la zone de texte et de la scrollbar horizontale
        self.symptom_frame = ctk.CTkFrame(self.frame_right, fg_color=self.theme[1], corner_radius=0)  # Cadre pour le texte et la scrollbar
        self.symptom_frame.pack(fill=ctk.BOTH, expand=True, padx=20, pady=20)

        # Création de la scrollbar horizontale
        self.text_scroll_horizontal = tk.Scrollbar(self.symptom_frame, orient='horizontal')
        self.text_scroll_horizontal.pack(side=tk.BOTTOM, fill=tk.X)

        # Modification de la zone de texte pour y intégrer la scrollbar horizontale
        text_width = self.fenetre.winfo_screenwidth() // 10

        # Configurer le widget Text avec cette largeur
        self.text_output = tk.Text(self.symptom_frame, height=40, width=text_width, wrap=tk.NONE, xscrollcommand=self.text_scroll_horizontal.set)
        self.text_output.pack(side=tk.LEFT, fill=ctk.BOTH, expand=True)

        # Configuration de la scrollbar pour qu'elle interagisse avec la zone de texte
        self.text_output.config(xscrollcommand=self.text_scroll_horizontal.set)
        self.text_scroll_horizontal.config(command=self.text_output.xview)
    ...
    def update_right_panel(self, text, is_start_time=False):
        ...
        self.text_output.insert(tk.END, "ZZZZZZZZZZZZ" * 100)
        self.text_output.update_idletasks()
...
class LecteurVideo():
    ...
    def afficher_video(self):
        ...
        if self.interface_generale.cap.isOpened():# Vérifie si la vidéo est ouverte
            ...
            if ret:
                # Mettre à jour la plage de la barre de progression
                ...
                self.interface_generale.progress_slider.config(from_=0, to=total_seconds)
                ...
                current_time = self.interface_generale.cap.get(cv2.CAP_PROP_POS_MSEC) / 1000.0
                self.interface_generale.progress_slider.set(current_time)
                # Démarrage de la mise à jour régulière de la position de la vidéo
                self.mettre_a_jour_position_video()
    ...
    def mettre_a_jour_position_video(self):
        """
        Met à jour régulièrement la position de la vidéo et la barre de progression.
        """
        if self.video_paused:
            return  # Ne faites rien si la vidéo est en pause

        if not self.interface_generale.cap.isOpened():
            return  # Ne faites rien si la vidéo n'est pas ouverte
        ...
        fps = self.interface_generale.cap.get(cv2.CAP_PROP_FPS)
        temps_ecoule = int(frame_number / fps)
        self.interface_generale.label_temps.config(text=f"Temps écoulé: {self.format_duree(temps_ecoule)} / Durée totale: {self.format_duree(self.interface_generale.get_video_duration())}")

        # Continuez la mise à jour toutes les 500 ms
        self.interface_generale.fenetre.after(500, self.mettre_a_jour_position_video)
