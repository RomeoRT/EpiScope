...
class FriseSymptomes:
    ...
    def afficher(self):
        ...
        for ligne in lignes_symptomes:
            parts = ligne.split(' - ')  # Sépare les différentes parties
            if len(parts) > 0:
                nom = parts[0]  # Prendre seulement la première partie, le nom du symptôme
                tdeb = random.randint(0,3)  # Simulez le début pour cet exemple
                tfin = random.randint(4,10)  # Simulez la fin
                # Ajouter les informations formatées à la liste des symptômes
                symptomes_formattes.append([nom, tdeb, tfin])
        # Triez et envoyez à la frise sans "TD" et "TF"
        symptomes_formattes.sort(key=lambda x: x[1])
        afficher_frise(symptomes_formattes)  # Assurez-vous que cette fonction attend les bons formats
...
class InterfaceGenerale():
    ...
    def update_right_panel(self, text, is_start_time=False):
        def set_end_time(event):
            ...
            if "- TF:" not in current_text:
                ...
                if len(parts) >= 2:  # Assurez-vous qu'il y a bien un temps de début pour insérer le temps de fin
                    new_text = f"{parts[0]} - {parts[1]} - TF: {self.get_current_video_time()}"
        ...
        if is_start_time:
            ...
            symptom_with_time = f"{text} - TD: {self.get_current_video_time()}"
        else:
            # Sinon, affichez seulement le nom du symptôme
            symptom_with_time = text

        symptom_label = tk.Label(self.text_output, text=symptom_with_time, bg="light grey", fg="black")
        symptom_label.bind('<Button-1>', set_end_time)  # Associe l'événement de clic pour ajouter le temps de fin
